...
public class Cinema implements Serializable,Observer
{
	...
	/*
	public ArrayList<Date>getAvailableDate(int theaterId) 
	{
		ArrayList<Date> nonUsedDates = new ArrayList<Date>();
		SimpleDateFormat sdf = new SimpleDateFormat("EEEE, MMM dd, yyyy HH:mm:ss a");
		...
		for(Screening i: screenlist)
		{
			if(i.getTheaterId() == theaterId)
			{
				for(Date j: cinemaManager.dateList)
				{

					if(!(sdf.format(j).equals(sdf.format(i.getDate()))))
					{
						nonUsedDates.add(j);
					}
				}
	...
	public ArrayList<String>getAvailableDateString(int theaterId) 
	{
		ArrayList<Date> dateList = getAvailableDate(theaterId);
		ArrayList<String> dateStringList = new ArrayList<String>();
		for(Date i:dateList)
		{
			SimpleDateFormat sdf = new SimpleDateFormat("EEEE, MMM dd, yyyy HH:mm:ss a");
			dateStringList.add(sdf.format(i));
		}
